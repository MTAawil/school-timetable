...
const periods = [
  { name: "Period 1", startsAt: "07:30", endsAt: "08:25", isTeaching: true },
  { name: "Period 2", startsAt: "08:25", endsAt: "09:20", isTeaching: true },
  { name: "Period 3", startsAt: "09:20", endsAt: "10:15", isTeaching: true },
  { name: "Recess", startsAt: "10:15", endsAt: "10:35", isTeaching: false },
  { name: "Period 4", startsAt: "10:35", endsAt: "11:30", isTeaching: true },
  { name: "Period 5", startsAt: "11:30", endsAt: "12:25", isTeaching: true },
  { name: "Period 6", startsAt: "12:25", endsAt: "13:20", isTeaching: true },
] as const;
...
function gradeForClass(shortCode: string): string {
  const ebGrade = /^EB(\d+)$/u.exec(shortCode);
  if (ebGrade) {
    return ebGrade[1];
  }
  const numericGrade = /^(\d+)/u.exec(shortCode);
  if (numericGrade) {
...
async function main(): Promise<void> {
  ...
  for (const detail of allTeacherDetails) {
    ...
    await db.teachingRequirement.create({
      data: {
        ...
        subjectId,
        teacherId,
        weeklyOccurrences: detail.hours,
        durationPeriods: 1,
        minOccurrencesPerDay: 0,
        maxOccurrencesPerDay: Math.min(2, detail.hours),
        minimumDistinctDays: Math.ceil(detail.hours / 2),
        allowMultipleOccurrencesSameDay: detail.hours > 1,
        notes:
          detail.sourceRow === 0
            ? "Temporary demo assignment for missing needed hours."
            : `Source row ${detail.sourceRow} from تفاصيل الحصص.`,
      },
    });
  }
  ...
  console.log(
    `Al Massar seed created: ${allClasses.length} classes, ${teacherIds.size} teachers, ${subjectIds.size} subjects, ${allTeacherDetails.length} teaching requirements.`,
  );
}
